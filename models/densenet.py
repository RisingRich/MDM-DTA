...
class DenseLayer(torch.nn.Module):
    def __init__(self, in_channels, middle_channels=128, out_channels=32, se_reduction=16):
...
class Transition(torch.nn.Sequential):
    def __init__(self, channels, se_reduction=16):
        ...
        self.conv = nn.Conv1d(channels, channels // 2, 3, padding=1)
        self.se = SELayer(channels // 2, reduction=se_reduction)
    ...
    def forward(self, x):
        ...
        x = self.conv(x)
        x = self.se(x)
        return nn.AvgPool1d(2)(x)


class DenseNet(torch.nn.Module):
    def __init__(self, layer_num=(6, 12, 24, 16), growth_rate=32, init_features=64, in_channels=1,
...
# x = torch.randn(2, 1, 256).to(device)
